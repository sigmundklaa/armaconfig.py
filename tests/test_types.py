...
def test_array_symbol():
    with pytest.raises(Unexpected) as e:
        loads('array[] = 1;')
...
def test_string_unquoted():
    assert loads('unquoted = unquoted string;') == {'unquoted': 'unquoted string'}

def test_string_joined():
    assert loads('joined = unquoted and "quoted" strings "joined" together;') == {'joined': 'unquoted and quoted strings joined together'}

def test_string_escaped():
    assert loads('escaped = "this ""string"" is ""escaped"".";') == {'escaped': 'this "string" is "escaped".'}